...
- `list_issues` - List and search workspace issues
...
You can use environment variables in your configuration to keep secrets out of version control and enable different values per environment.
...
url: ${MCP_URL:-https://api.example.com/mcp}
...
- Use `${VAR_NAME}` for required variables (deployment fails if not set)
- Use `${VAR_NAME:-default}` for optional variables with fallback values
- Interpolation happens before validation, so values must match expected types

### Third-Party MCP Servers

The most common use case is providing API keys for third-party MCP servers:
  ...
  model:
    anthropic:
      mcp_servers:
        ...
        - type: url
          url: ${KNOWLEDGE_BASE_URL:-https://api.ragie.ai/v1/mcp}
          name: 'Knowledge Base'
          authorization_token: ${RAGIE_API_KEY}
```

<Warning>
  **Never commit secrets to version control.** Always use environment variables for API keys,
...
```bash
# .env
RAGIE_API_KEY=your_secret_key_here
KNOWLEDGE_BASE_URL=https://api.ragie.ai/v1/mcp
```
...
**Production** - use your platform's secrets management (AWS Secrets Manager, Vercel env vars, etc.)
