...
flowchart LR
  ...
  Proxy <-->|WebSocket| CLI[Sniff CLI<br/>Your Machine]
...
| **Proxy**            | Cloudflare Worker that handles OAuth and routes webhooks         |
| **Sniff CLI**        | Connects to proxy via WebSocket and runs agents locally          |
| **Claude Agent SDK** | Executes agents with tools (Read, Write, Edit, Bash, etc.)       |

The CLI initiates an outbound WebSocket connection to the proxy - no tunnels or port forwarding needed.
...
The proxy verifies the signature (if configured) and pushes the webhook to your CLI through the WebSocket connection.
...
- `GET /connect` - CLI connects here via WebSocket for real-time webhook delivery

### Webhook Routing

- `POST /webhook/linear` - Receive Linear webhook, route to connected CLI

### Health Check

- `GET /` or `GET /health` - Returns "OK"

The proxy uses Durable Objects to maintain WebSocket connections. Each Linear organization gets its own connection handler.
